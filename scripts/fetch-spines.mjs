...
async function getSpineUrl(title) {
  ...
  try {
    ...
    // coversum/coverlist/cover500 → spineflip, _N.jpg → _d.jpg
    return coverUrl
      .replace(/cover(sum|list|500)/, 'spineflip')
      .replace(/_\d+\.jpg/, '_d.jpg')
  } catch {
    return null
  }
}
...
function download(url, dest) {
  return new Promise((resolve, reject) => {
    ...
    proto.get(url, { headers: { 'User-Agent': 'Mozilla/5.0 Chrome/120' } }, res => {
...
async function main() {
  ...
  for (const book of BOOKS) {
    ...
    try {
      const spineUrl = await getSpineUrl(book.title)
      if (!spineUrl) { console.log(`❌ not found  ${book.id}`); continue }
      await download(spineUrl, dest)
      const size = fs.statSync(dest).size
      console.log(`✅ saved  ${book.id}  (${size}B)  ← ${spineUrl}`)
    } catch (e) {
